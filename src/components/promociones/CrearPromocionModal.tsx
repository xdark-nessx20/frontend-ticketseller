...
import type { MecanismoAplicacion, TipoUsuario } from '../../types/promociones.types';
...
export function CrearPromocionModal({ eventoId, onClose }: CrearPromocionModalProps) {
  ...
  function onSubmit(data: FormValues) {
    mutate(
      {
        nombre: data.nombre,
        mecanismo: data.mecanismo as MecanismoAplicacion,
        eventoId,
      ...
      <div className="w-full max-w-lg rounded-lg bg-white p-6 shadow-xl">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="mt-4 space-y-4">
          ...
          <div>
            ...
            {errors.mecanismo && <p className="mt-1 text-xs text-red-600">{errors.mecanismo.message}</p>}
          </div>
