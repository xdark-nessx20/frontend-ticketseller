...
export function EditarEventoModal({ evento, onClose }: EditarEventoModalProps) {
  const { mutate, isPending } = useEditarEvento(evento.id);
      ...
      },
      {
        ...
        onError: err => {
          if (axios.isAxiosError(err) && err.response?.status === 409) {
            setError('root', {
              message:
                err.response.data?.message ?? 'No se puede editar un evento en progreso.',
            });
          } else {
